...
export default function Dashboard() {
  ...
  const stats = [
    { icon: <MessageCircle size={18} />, label: t("dashboard.aiConversations"), value: '—', color: '#C8102E' },
    { icon: <FileText size={18} />, label: t("dashboard.savedDocuments"), value: '—', color: '#2563EB' },
    { icon: <Bookmark size={18} />, label: t("dashboard.bookmarks"), value: '—', color: '#D97706' },
    { icon: <Heart size={18} />, label: t("dashboard.communityPosts"), value: '—', color: '#059669' },
  ];

  const quickActions = [
    { icon: <Sparkles size={18} />, label: t("dashboard.askAI"), to: '/chat', color: 'var(--primary)' },
    { icon: <FileText size={18} />, label: t("dashboard.generateDoc"), to: '/chat', color: '#2563EB' },
    { icon: <Heart size={18} />, label: t("dashboard.community"), to: '/confessions', color: '#059669' },
    { icon: <Bell size={18} />, label: t("common.notifications"), to: '/notifications', color: '#D97706' },
    { icon: <FileText size={18} />, label: t("dashboard.legalDocuments"), to: '/documents', color: '#7C3AED' },
    { icon: <Bookmark size={18} />, label: t("dashboard.legalGlossary"), to: '/glossary', color: '#0891B2' },
  ];

  const recentActivity = [
    { icon: <User size={14} />, text: t("dashboard.accountCreated"), time: t("common.justNow") },
  ];
    ...
    <div className="dash-page">
      <div className="dash-sidebar">
        ...
        <nav className="dash-nav">
          {[
            { key: 'overview', icon: <Activity size={16} />, label: t("dashboard.overview") },
            { key: 'activity', icon: <Clock size={16} />, label: t("dashboard.activity") },
            { key: 'settings', icon: <Settings size={16} />, label: t("common.settings") },
          ].map(item => (
            <button
              key={item.key}
        ...
        {tab === 'activity' && (
          <div>
            ...
            <div className="dash-empty" style={{ marginTop: 40 }}>
              <Activity size={40} />
              <p>{t("dashboard.noActivityDesc")}</p>
              <button className="btn btn-primary" onClick={() => navigate('/chat')} style={{ marginTop: 16 }}>
                {t("dashboard.startChatting")} <ArrowRight size={16} />
              </button>
            </div>
          </div>
        )}
